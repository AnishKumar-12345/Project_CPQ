<template>
    <div>
            ...
            >
              <template v-slot:top>
                <v-toolbar flat>
                  ...
                  <v-icon color="red" large @click="addItemMake">mdi-plus</v-icon>
                </v-toolbar>
              </template>
      ...
      <v-dialog v-model="dialog2" max-width="600px">
        <v-card style="background-color:rgba(223,240,254,255);border:3px solid rgba(138,222,199,255);border-radius:4px;">
          <v-card-title>Add Tool Make</v-card-title>
          <v-card-text>
            <v-form v-model="valid" ref="form" @submit.prevent="saveToolMakes">
              <v-row>
                <v-col cols="12" sm="6" md="6">
                  <v-text-field
                    label="Tool Make Code"
                    placeholder="Tool Make Code"
                    :rules="nameRules"
                ...
                <v-col cols="12" sm="6" md="6">
                  <v-text-field
                    label="Tool Make Name"
                    placeholder="Tool Make Name"
                ...
                <v-card-actions class="mr-3">
                  ...
                  style="width:80px;" @click="cancelItemMake()"
                    >Cancel</v-btn
                  >
      ...
      <v-dialog v-model="dialogDelete" max-width="600px" persistent>
        <v-card style="background-color:#fde4e4;border:3px solid rgba(243,145,146,255);border-radius:4px;padding:15px;">
          <v-card-title  style="font-size: 20px"
            >Are you sure you want to delete Tool Make ?</v-card-title
          >
          <v-card-actions class="mr-3">
            <v-spacer></v-spacer>
            <v-btn  color="red white--text"
  ...
  export default {
    ...
    computed: {
      formTitle() {
        return this.editedIndex === -1 ? "Add Tool make" : "Edit Tool make";
      },
    },
    mounted() {
      this.getAllMakes();
    ...
    methods: {
      ...
      submitFIle() {
        if (this.file == "") {
          ...
          if (
            ...
            this.uploadToolMakes(formData).then((response) => {
      ...
      confirmToDelete() {
        ...
        this.deleteAllToolMake(reqJSON).then((response) => {
          if (response.status == 200) {
            this.text = response.data.message;
            this.activeColor = "green";
            this.snackbar = true;
      ...
      saveToolMakes() {
        if (this.$refs.form.validate()) {
          ...
          this.saveToolMake(reqJson).then((response) => {
            this.snackbar = false;
            if (response.status == 200) {
              this.text = response.data.message;
              this.activeColor = "green";
