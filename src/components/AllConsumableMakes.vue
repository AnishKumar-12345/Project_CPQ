    ...
    <div>
            ...
            >
              <template v-slot:top>
                <v-toolbar flat>
                  ...
                  <v-icon color="red" large @click="addConsumableMake">mdi-plus</v-icon>
                </v-toolbar>
              </template>
  
              <template v-slot:item.actions="{ item }">
                <v-icon small class="mr-2 blue--text" @click="editConsumableCode(item)">
                  mdi-pencil
                </v-icon>
      ...
      <v-dialog v-model="dialog2" max-width="600px">
        <v-card style="background-color:rgba(223,240,254,255);border:3px solid rgba(138,222,199,255);border-radius:4px;">
          <v-card-title>Add Consumable Makes</v-card-title>
          <v-card-text>
            <v-form v-model="valid" ref="form" @submit.prevent="saveConsumableMakes">
              <v-row>
                <v-col cols="12" sm="6" md="6">
                  <v-text-field
                    label="Consumable Make Code"
                    placeholder="Consumable Make Code"
                    :rules="nameRules"
                ...
                <v-col cols="12" sm="6" md="6">
                  <v-text-field
                    label="Consumable Make Name"
                    placeholder="Consumable Make Name"
                ...
                <v-card-actions class="mr-3">
                  ...
                  <v-btn color="red white--text"
                              style="width:80px;margin-left:10px;" @click="cancelConsumableMake()"
                    >Cancel</v-btn
                  >
      ...
      <v-dialog v-model="dialog" max-width="600">
        <v-card style="background-color:rgba(223,240,254,255);border:3px solid rgba(138,222,199,255);border-radius:4px;">
          ...
          <v-form ref="form" @submit.prevent="editConsumableMake">
            <v-card-text>
      ...
      <v-dialog v-model="dialogDelete" max-width="700px" persistent>
        <v-card style="background-color:#fde4e4;border:3px solid rgba(243,145,146,255);border-radius:4px;padding:15px;">
          <v-card-title style="font-size: 20px"
            >Are you sure you want to delete Consumable Make ?</v-card-title
          >
          <v-card-actions class="mr-3">
            <v-spacer></v-spacer>
            <v-btn  color="red white--text"
  ...
  export default {
    ...
    data() {
      return {
        ...
        allMakes: [],
        headers: [
          { text: "Consumable Make Code", value: "makeCode" },
          { text: "Consumable Make Name", value: "makeName" },
    ...
    computed: {
      formTitle() {
        return this.editedIndex === -1 ? "Add Consumable make" : "Edit Consumable make";
      },
    },
    mounted() {
      this.getAllMakes();
    ...
    methods: {
      ...
      submitFIle() {
        if (this.file == "") {
          ...
          if (
            ...
            this.uploadConsumableMakes(formData).then((response) => {
      ...
      confirmToDelete() {
        ...
        this.deleteAllConsumableMake(reqJSON).then((response) => {
      ...
      saveConsumableMakes() {
        if (this.$refs.form.validate()) {
          ...
          this.saveConsumableMake(reqJson).then((response) => {
            this.snackbar = false;
            if (response.status == 200) {
              this.text = response.data.message;
              this.activeColor = "green";
