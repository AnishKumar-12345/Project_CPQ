          ...
          >
            ...
            <v-spacer></v-spacer>
                <vue-excel-xlsx
                    :data="fittingtypemap"
                    :columns="jsonfields"
                    :file-name="'Item Fitting Mapping'"
                    :file-type="'xlsx'"
                 ...
                 <vue-excel-xlsx :data="json_data" :columns="json_data"
                    :file-name="'Item Fitting Mapping'"
                    :file-type="'xlsx'"
                    :sheet-name="'sheetname'"  v-if=" fittingtypemap == ''">                    
        ...
        <div v-if="this.activeUpload==true">
         <v-dialog
          ...
          v-model="activeUpload" >
           ...
           <v-card>
             <v-card-title style="background-color:#2cb1fe;color:white;">Upload Item Type Map</v-card-title>
           ...
           uploadFile(event){
            ...
            submitFIle(){
                if(this.file ==''){
                   ...
                   formData.append('item-type-fitting-type-map',this.file)
                         this.uploadFittingTypeMap(formData).then((response) => {
          ...
          this.addItem(reqJSON).then(response => {
            ...
            setInterval(this.close(), 3000);
               ...
               getItems(){
                    ...
                    this.getAllfittingtypemap().then((response) => {
            
               this.fittingtypemap=response  
            }
            )},
