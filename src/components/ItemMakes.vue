    ...
    <div>     
        ...
        ></v-text-field>
                    ...
                    <v-spacer></v-spacer>
                        ...
                        :file-name="'Item Makes'"
                        :file-type="'xlsx'"
                        :sheet-name="'sheetname'" v-if="allMakes == ''">                    
      ...
      <v-icon
        small
        class="mr-2 blue--text"   @click="editItemCode(item)" >
        mdi-pencil
        ...
        mdi-delete
          ...
          <v-dialog v-model="dialog2" max-width="600px">
             <v-card  style="background-color:rgba(223,240,254,255);border:3px solid rgba(138,222,199,255);border-radius:4px;">
                <v-card-title>Add Item Make</v-card-title>
                <v-card-text>
                    <v-form 
                    v-model="valid" ref="form"
                    @submit.prevent="saveItemMakes"
                ...
                <v-row>
                  ...
                  >
                         <v-text-field
                           label="Item Make Code"
                           placeholder="Item Make Code"
                           :rules="nameRules"
          ...
          <v-dialog v-model="dialog"  max-width="600">
            <v-card  style="background-color:rgba(223,240,254,255);border:3px solid rgba(138,222,199,255);border-radius:4px;">
              <v-card-title>Edit Item Code</v-card-title>
               <v-form ref="form" @submit.prevent="editItemMake">
              <v-card-text>
          ...
          <v-card style="background-color:rgba(253,228,228,255);border:3px solid rgba(243,145,146,255);border-radius:4px;">
            <v-card-title  style="font-size:20px">Are you sure you want to delete Item Make ?</v-card-title>
            <v-card-actions class="mr-3">
              <v-spacer></v-spacer>
              <v-btn  color="red white--text"
                            style="width:80px;margin-left:10px;" @click="dialogDelete=false">Cancel</v-btn>
...
export default {
   ...
   data(){
       return{
        ...
        search:'',
           ...
           allMakes:[],
           headers:[
               {text:"Item Make Code", value:"makeCode"},
               {text:"Item Make Name", value:"makeName"},
   ...
   computed: {
      formTitle () {
        return this.editedIndex === -1 ? 'Add Item make' : 'Edit Item make'
      },
    },
   mounted(){
       this.getAllMakes();
    ...
    submitFIle(){
            if(this.file ==''){
               ...
               this.isUploadCard=true;
                     this.uploadItemMakes(formData).then((response) => {                      
           ...
           confirmToDelete(){
              ...
              var reqJSON=this.deleteModel
                this.deleteAllItemMake(reqJSON).then(response => {
                    if(response.status==200){                   
                     this.text=response.data.message;
                     this.activeColor='green'
                     this.snackbar=true;
       ...
       saveItemMakes(){
            ...
            if(this.$refs.form.validate()){
             ...
             this.saveItemMake(reqJson).then((response) => {
                  this.snackbar=false;
                 if(response.status==200){
