<template>
          ...
          >
            ...
            <v-spacer></v-spacer>
              <vue-excel-xlsx
                    class="btn"
                      :data="result"
                      :columns="jsonfields" :file-type="'xlsx'" :file-name="'Accessories '" :sheet-name="'sheetname'"
                      v-if="result != ''"
                    >
                  <v-icon
                  large
                 ...
                 <vue-excel-xlsx :data="json_data" :columns="json_data" :file-type="'xlsx'" :file-name="'Accessories '" :sheet-name="'sheetname'" v-if="result == ''">                    
    ...
    <v-form ref="form" v-model="valid" lazy-validation @submit.prevent="saveItem"  > 
            ...
            <v-dialog v-model="dialogDelete" max-width="650px">
              <v-card style="background-color:#fde4e4;border:3px solid rgba(243,145,146,255);border-radius:4px;padding:15px;">
                <v-card-title  style="font-size:20px ">Are you sure you want to delete  Accessories Type ?</v-card-title>
                <v-card-actions>
                  <v-spacer></v-spacer>
         ...
         <template v-slot:item.actions="{ item }">
                  <v-icon small color="error" class="mr-2" @click="deleteitem(item)"> mdi-delete </v-icon>
    ...
    <div v-if="this.activeUpload==true">
         <v-dialog
          ...
          v-model="activeUpload" >
           ...
           <v-card>
             <v-card-title style="background-color:#2cb1fe;color:white;margin-top:-5px;">Upload Accesories</v-card-title>
    ...
    export default {
        ...
        data(){
            return {
              filename:'AllAccessorieslist',
              jsonfields:[{
                label: "Accessorires_Type_Code",
               field: "fittingTypeCode",
              },
           {
            label: "Accessorires_Name",
               field: "fittingName",
           },
           {
            label: "Material_Type",
      ...
      "unit3_type": "",
        ...
        methods: {
           ...
           uploadItem(event){
            ...
            submitFile(){
                   ...
                   formData.append('accessorires',this.file)
                   this.isProgress=true;
                   this.isUploadCard=true;
                this.uploadAccessories(formData).then((response) => {
                  ...
                  activeUploadCard(){
                    ...
                    confirmToDelete(){
                     var reqJSON= this.delItem
                       this.deleteaccessorires(reqJSON).then((response) => {
                    if(response.status==200){
                         this.text=response.data.message;
                         this.snackbar=true
                         this.activeColor='';
            ...
            saveItem(){
                  this.$refs.form.validate()
                   //console.log("json",this.editedCategory,this.valid);
                     this.postItem()
        ...
        //console.log("1234",this.editedCategory)
        ...
        //  //console.log("check", reqJSON);
          this.addItem(reqJSON).then(response => {
            ...
            //setInterval(this.close(), 3000);
               ...
               //console.log("Get All vendors",response.length)
    //console.log("result",response[0].categoryName)
                  for(var i=0;i<response.length;i++){
                    this.categoryList.push(response[i].categoryName)
                  }
              // this.result=response;
               //console.log("result1",this.categoryList)
            ...
            setInterval(this.close(), 3000);
               ...
               getItems(){
                    ...
                    this.getAllAccessoriesList().then((response) => {
               this.result=response  
            }
            )},
