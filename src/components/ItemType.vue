          ...
          <tr>
            ...
            <td style="text-align: left">
              <router-link
                :to="{
                  name: 'AddNewItem',
                  params: { itemTypeCode: row.item.itemTypeCode,},
                }"
              >
            ...
            <td>
              <router-link
                :color="getColor(row.item.isActiveItems)"
                :to="{
                  name: 'AddNewItem',
                  params: { itemTypeCode: row.item.itemTypeCode, tabValue: 1 },
                }"
              >
                {{ row.item.isActiveItems ? "Yes" : "No" }}
              </router-link>
            </td>
            <td>
              <router-link
                :color="getColor(row.item.isActiveItemFittings)"
                :to="{
                  name: 'AddNewItem',
                  params: { itemTypeCode: row.item.itemTypeCode, tabValue: 3 },
                }"
              >
            ...
            <td style="text-align: left">
              <v-icon class="red--text" small @click="deleteitem(row.item)"
                >mdi-delete</v-icon
              >
            </td>
          </tr>
        ...
        <template v-slot:top>
          <v-toolbar flat>
            ...
            <vue-excel-xlsx
              ...
              v-if="result == ''"
            >
              <v-icon large color="green darken-2"> mdi-cloud-download </v-icon>
            </vue-excel-xlsx>
            <v-icon
            ...
            <v-dialog v-model="dialogDelete" max-width="600px">
              <v-card style="background-color:#fde4e4;border:3px solid rgba(243,145,146,255);border-radius:4px;padding:15px;">
                <v-card-title  style="font-size: 20px;"
                  >Are you sure you want to delete Item Type ?</v-card-title
                >
                <v-card-actions>
                  <v-spacer></v-spacer>
                  ...
                  <v-btn color="green white--text"
                              style="width:80px;"  @click="confirmToDelete"
                    >OK</v-btn
                  >
                  <v-spacer></v-spacer>
                </v-card-actions>
        ...
        <v-dialog v-model="dialog1" persistent max-width="600px">
          <v-card>
            ...
            <v-card-actions>
              ...
              <v-btn color="green darken-1" text @click="saveAll"> save </v-btn>
            </v-card-actions>
          </v-card>
        </v-dialog>
      </div>
        ...
        <v-dialog persistent max-width="600px" v-model="activeUpload">
          <v-card>
            <v-card-title style="background-color: #2cb1fe;color:white;"
              >Upload Item Types</v-card-title
            >
  ...
  export default {
    ...
    data() {
      return {
        ...
        headers: [
          ...
          {
            text: "Active Fittings",
            value: "isActiveItemFittings",
    ...
    mounted() {
      ...
      if (this.$route.params.itemTypeCode) {
        this.makeitem = this.$route.params.itemTypeCode;
        this.makeitem = this.makeitem.replace("/", "$");
        this.getItem();
      }
    },
    methods: {
      ...
      submitFIle() {
        if (this.file == "") {
          ...
          if (
            ...
            this.uploadItemType(formData).then((response) => {
      ...
      confirmToDelete() {
        ...
        this.deleteItemType(reqJSON).then((response) => {
          if (response.status == 200) {
            this.text = response.data.message;
            this.snackbar = true;
            this.activeColor = "";
