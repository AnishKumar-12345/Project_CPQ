<template>
    <div>
            ...
            >
              <template v-slot:top>
                <v-toolbar flat>
                  ...
                  <v-icon color="red" large @click="addFittingMake"
                    >mdi-plus</v-icon
                  >
                </v-toolbar>
              </template>
              ...
              <template v-slot:item.actions="{ item }">
                <v-icon small class="mr-2 blue--text" @click="editItemCode(item)">
                  mdi-pencil
                </v-icon>
      ...
      <!-- add Item Make form start -->
      <v-dialog v-model="dialog2" max-width="600px">
        <v-card style="background-color:rgba(223,240,254,255);border:3px solid rgba(138,222,199,255);border-radius:4px;">
          <v-card-title>Add Fitting Make</v-card-title>
          <v-card-text>
            <v-form v-model="valid" ref="form" @submit.prevent="saveFittingMakes">
              <v-row>
                ...
                <v-card-actions class="mr-3">
                  ...
                  <v-btn color="red white--text"
                              style="width:80px;margin-left:10px;" @click="cancelFittingMake()"
                    >Cancel</v-btn
                  >
      ...
      <v-dialog v-model="dialog" max-width="600">
        <v-card style="background-color:rgba(223,240,254,255);border:3px solid rgba(138,222,199,255);border-radius:4px;">
          ...
          <v-form ref="form" @submit.prevent="editFittingMake">
            <v-card-text>
              <v-row justify="space-around" class="ma-3">
                <v-col cols="12" sm="12" md="6">
                  <v-text-field
                    ...
                    outlined
                    dense
                    placeholder="Fittng Make Code"
                    label="Fittng Make Code"
                ...
                <v-col cols="12" sm="12" md="6">
                  <v-text-field
                    outlined
                    dense
                    placeholder="Fittng Make Name"
                    label="Fittng Make Name"
      ...
      <v-dialog v-model="dialogDelete" max-width="650px" persistent>
        <v-card style="background-color:#fde4e4;border:3px solid rgba(243,145,146,255);border-radius:4px;padding:15px;">
          <v-card-title style="font-size: 20px"
            >Are you sure you want to delete Fitting Make ?</v-card-title
          >
          <v-card-actions class="mr-3">
            <v-spacer></v-spacer>
            <v-btn color="red white--text"
  ...
  export default {
    ...
    data() {
      return {
        ...
        allMakes: [],
        headers: [
          { text: "Fitting Make Code", value: "makeCode" },
          { text: "Fitting Make Name", value: "makeName" },
    ...
    computed: {
      formTitle() {
        return this.editedIndex === -1 ? "Add Fitting make" : "Edit Fitting make";
      },
    },
    mounted() {
      this.getAllMakes();
    ...
    methods: {
      ...
      submitFIle() {
        if (this.file == "") {
          ...
          if (
            ...
            this.uploadFittingMakes(formData).then((response) => {
      ...
      confirmToDelete() {
        ...
        this.deleteAllFittingMake(reqJSON).then((response) => {
          if (response.status == 200) {
            this.text = response.data.message;
            this.activeColor = "green";
            this.snackbar = true;
      ...
      saveFittingMakes() {
        if (this.$refs.form.validate()) {
          ...
          this.saveFittingMake(reqJson).then((response) => {
            this.snackbar = false;
            if (response.status == 200) {
              this.text = response.data.message;
              this.activeColor = "green";
