<template>
         ...
         <template v-slot:item="row" >
             <tr >
               ...
               <td style="text-align:left">
                 ...
                 <router-link :to="{ name: 'ConsumableSearch', params: { fittingTypeCode: row.item.fittingTypeCode  } }" style=" text-decoration: none">
    ...
    <v-form ref="form" v-model="valid" lazy-validation @submit.prevent="saveItem"  > 
            ...
            <v-dialog v-model="dialogDelete" max-width="700px">
              <v-card style="background-color:#fde4e4;border:3px solid rgba(243,145,146,255);border-radius:4px;padding:15px;">
                <v-card-title style="font-size:20px ">Are you sure you want to delete  Consumable Type ?</v-card-title>
                <v-card-actions>
                  <v-spacer></v-spacer>
    ...
    export default {
        ...
        data(){
            return {
              filename:'AllConsumableTypes',
              json_data: [{
            label: "Consumable_Type_Code",
               field: "",
            },{
            label: "Consumable_Type_Name",
               field: "",
          },
          {
            label: "Sub_Category",
          ...
          headers: [
            ...
            {text: 'Consumable Type Code ', value: 'fittingTypeCode'},
            { text: 'Consumable Type Name', value: 'fittingTypeName' },
        ...
        methods: {
            ...
            submitFIle(){
                if(this.file ==''){
                   ...
                   this.isUploadCard=true;
                         this.uploadConsumableType(formData).then((response) => {
                  ...
                  activeUploadCard(){
                    ...
                    confirmToDelete(){
                     ...
                     var reqJSON= this.delItem
                       this.deleteConsumableType(reqJSON).then((response) => {
                    if(response.status==200){
                         this.text=response.data.message;
                         this.snackbar=true
                         this.activeColor='';
          ...
          this.addItem(reqJSON).then(response => {
            ...
            setInterval(this.close(), 3000);
                  ...
                  //GET API Calls
               getConsumables(){
                    this.getAllConsumable().then((response) => {
