<template>
      ...
      <v-tabs-items v-model="tab">
        <v-tab-item>
                ...
                <template v-slot:top>
                  <v-toolbar flat>
                    ...
                    <vue-excel-xlsx
                          :data="allCategory"
                          :columns="jsonfields"
                          :file-name="'Accessories Category'"
                          :file-type="'xlsx'"
                    ...
                    <v-icon color="red" large @click="addAccessoriesMake"
                      >mdi-plus</v-icon
                    >
                  </v-toolbar>
                </template>
          ...
          <v-dialog v-model="dialog2" max-width="600px">
            <v-card style="background-color:rgba(223,240,254,255);border:3px solid rgba(138,222,199,255);border-radius:4px;">
              <v-card-title>Add Accessories Category</v-card-title>
              <v-card-text>
                <v-form
                  v-model="valid"
                  ref="form"
                  @submit.prevent="saveAccessoriresCategories"
                >
                  <v-row>
                    <v-col cols="12" sm="6" md="6">
                      <v-text-field
                        label="Accessories Category Name"
                        placeholder="Accessories Category Name"
                        :rules="nameRules"
                        v-model="addAccessoriesCategory.categoryName"
                        dense
                        outlined
                      >
                      </v-text-field>
                    ...
                    <v-col cols="12" sm="6" md="6">
                      <v-text-field
                        label="Margin"
                        placeholder="Margin"
                        v-model="addAccessoriesCategory.margin"
                        :rules="nameRules"
                        dense
                        outlined
                      >
          ...
          <v-dialog v-model="dialog" max-width="600">
            <v-card   style="background-color:rgba(223,240,254,255);border:3px solid rgba(138,222,199,255);border-radius:4px;">
              ...
              <v-form ref="form" @submit.prevent="editAccessoriescategories">
                <v-card-text>
                  <v-row justify="space-around" class="ma-3">
                    <v-col cols="12" sm="12" md="6">
                      <v-text-field
                        ...
                        placeholder="Accessories Category Name"
                        label="Accessories Category Name"
                        v-model="editAccessoriesCategory.categoryName"
                      ></v-text-field>
                    </v-col>
                    <v-col cols="12" sm="12" md="6">
                      <v-text-field
                        outlined
                        dense
                        placeholder="Accessories Margin"
                        label="Accessories Margin"
                        v-model="editAccessoriesCategory.margin"
                      ></v-text-field>
                    </v-col>
                  </v-row>
                </v-card-text>
        ...
        <v-tab-item>
                ...
                >
                  <template v-slot:top>
                    <v-toolbar flat>
                      ...
                      <vue-excel-xlsx
                          :data="allSubCategory"
                          :columns="jsonfields2"
                          :file-name="'Accessories Sub Category'"
                          :file-type="'xlsx'"
                          :sheet-name="'sheetname'"
                             v-if="allSubCategory != ''"
                          >
                      ...
                      <v-icon color="red" large @click="addAccessoriesSubMake"
                        >mdi-plus</v-icon
                      >
                    </v-toolbar>
                  </template>
                  ...
                  <template v-slot:item.actions="{ item }">
                    <v-icon
                      small
                      class="mr-2 blue--text"
                      @click="editAccessoriesSubcat(item)"
                    >
                      mdi-pencil
                    </v-icon>
                    ...
                    <v-icon
                      class="pa-2 red--text"
                      small
                      @click="deleteSubAccessoriesCard(item)"
                    >
                      mdi-delete
                    </v-icon>
                  </template>
              ...
              <v-card style="background-color:rgba(223,240,254,255);border:3px solid rgba(138,222,199,255);border-radius:4px;">
                ...
                <v-card-text>
                  <v-form
                    v-model="valid"
                    ref="form1"
                    @submit.prevent="saveAccessoriresSubCategories"
                  >
                    <v-row>
                      <v-col cols="12" sm="6" md="6">
                        <v-autocomplete
                          ...
                          placeholder="Accessories Category Name"
                          :rules="nameRules"
                          v-model="addAccessoriresSubCategory.categoryName"
                      ...
                      <v-col cols="12" sm="6" md="6">
                        <v-text-field
                          ...
                          v-model="addAccessoriresSubCategory.subCategoryName"
                          :rules="nameRules"
                          dense
                          outlined
                        >
            ...
            <v-dialog v-model="dialog3" max-width="600">
              <v-card  style="background-color:rgba(223,240,254,255);border:3px solid rgba(138,222,199,255);border-radius:4px;">
                <v-card-title>Edit Accessories Code</v-card-title>
                <v-form ref="form" @submit.prevent="editAccessoriresSubCategories">
                  <v-card-text>
                    <v-row justify="space-around" class="ma-3">
                      <v-col cols="12" sm="12" md="6">
                        <v-text-field
                          ...
                          label="Category name"
                          v-model="editAccessoriesSubCategory.categoryName"
                        ></v-text-field>
                      </v-col>
                      <v-col cols="12" sm="12" md="6">
                        <v-text-field
                          ...
                          v-model="editAccessoriesSubCategory.subCategoryName"
                        ></v-text-field>
                      </v-col>
                    </v-row>
                  </v-card-text>
            ...
            <v-dialog v-model="dialogDelete2" max-width="800px" persistent>
              <v-card style="background-color:#fde4e4;border:3px solid rgba(243,145,146,255);border-radius:4px;padding:15px;">
                <v-card-title style="font-size: 20px"
                  >Are you sure you want to delete Sub Accessories Category
                  ?</v-card-title
                >
                <v-card-actions class="mr-3">
                  <v-spacer></v-spacer>
                  ...
                  <v-btn
                    ...
                    style="width:80px;"
                    class="mr-3"
                    @click="confirmSubAccessoriesToDelete"
                    >OK</v-btn
                  >
                  <v-spacer></v-spacer>
                </v-card-actions>
            ...
            <div v-if="this.activeUpload == true">
              <v-dialog persistent max-width="600px" v-model="activeUpload">
                <v-card>
                  <v-card-title style="background-color:#2cb1fe;color:white;"
                    >Upload Accessories Sub Category
                  </v-card-title>
                  <v-card-text class="mt-4">
                    <v-row align="center">
                      <v-col>
  ...
  export default {
    ...
    methods: {
      getscategory() {
        this.getAccessoriresCategory().then((response) => {
      ...
      addAccessoriesSubMake(){
        this.dialog4 = true;
      },
      cancelAccessoriesMake() {
        this.dialog2 = false;
        this.addAccessoriesCategory = {};
        this.$refs.form.reset();
        // this.makeModel = [];
      },
      cancelAccessorieSubMake(){
           this.dialog4 = false;
           this.addAccessoriresSubCategory = {};
      ...
      submitFIle() {
        if (this.file == "") {
          ...
          if (
            ...
            this.uploadAccessoriresCategories(formData).then((response) => {
      ...
      submitFIle2() {
        if (this.file == "") {
          ...
          if (
            ...
            formData.append("categories", this.file);
            this.isUploadCard = true;
            this.uploadAccessoriresSubCategories(formData).then((response) => {
              if (response.status == 200) {
                this.text = response.data.message;
                this.isProgress = false;
                this.activeColor = "green";
      ...
      confirmToDelete() {
        ...
        this.deleteAccessoriresCategory(reqJSON).then((response) => {
          if (response.status == 200) {
            this.text = response.data.message;
            this.activeColor = "green";
            this.snackbar = true;
      ...
      deleteSubAccessoriesCard(item) {
        this.deleteModel2 = item;
        this.dialogDelete2 = true;
      },
      confirmSubAccessoriesToDelete() {
        var reqJSON = this.deleteModel2;
        this.deleteAccessoriresSubCategory(reqJSON).then((response) => {
          if (response.status == 200) {
            this.text = response.data.message;
            this.activeColor = "green";
            this.snackbar = true;
      ...
      //edit item code
      editAccessoriescat(item) {
        this.editAccessoriesCategory = item;
        this.dialog = true;
      },
      editAccessoriesSubcat(item) {
        this.editAccessoriesSubCategory = item;
        this.dialog3 = true;
      },
      saveAccessoriresCategories() {
        if (this.$refs.form.validate()) {
          var reqJson = this.addAccessoriesCategory;
          this.saveAccessoriresCategory(reqJson).then((response) => {
            this.snackbar = false;
            if (response.status == 200) {
              this.text = response.data.message;
              this.activeColor = "green";
      ...
      saveAccessoriresSubCategories() {
        if (this.$refs.form1.validate()) {
          var reqJson = this.addAccessoriresSubCategory;
          this.saveAccessoriresSubCategory(reqJson).then((response) => {
            this.snackbar = false;
            if (response.status == 200) {
              this.text = response.data.message;
              this.activeColor = "green";
      ...
      editAccessoriescategories() {
        if (this.$refs.form.validate()) {
          var reqJson = this.editAccessoriesCategory;
          this.saveAccessoriresCategory(reqJson).then((response) => {
            this.snackbar = false;
            if (response.status == 200) {
              this.text = response.data.message;
      ...
      editAccessoriresSubCategories() {
        if (this.$refs.form.validate()) {
          var reqJson = this.editAccessoriesSubCategory;
          this.saveAccessoriresSubCategory(reqJson).then((response) => {
            this.snackbar = false;
            if (response.status == 200) {
              this.text = response.data.message;
